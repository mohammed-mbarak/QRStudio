...
A full-stack QR code generator application built with React.js (Vite) for the frontend and Node.js + Express + MongoDB for the backend. Generate customizable QR codes for text, URLs, emails, WiFi, phone, and vCard with storage and history support.
...
Zod / Joi for request validation (optional)
...
PORT=5000
MONGO_URI=mongodb+srv://<username>:<password>@cluster.mongodb.net/qrappdb
...
Serve the production frontend with Vercel, Netlify, or Nginx. Make sure VITE_API_URL points to the deployed backend URL.
...
Health check endpoint, QR generation, fetching by ID, pagination, and error handling scenarios are covered.
...
Components like QRGenerator, QRPreview, and InputField are tested. API calls are mocked, and loading, error, and success states are verified.
